...
import { AppState, Story, Question, Word } from './types';
import { prepareMandarinTTS, prepareConversationTTS, AudioController, SegmentTiming, initAudioContext } from './services/ttsService';

interface AppProps {
  onBackToSelection?: () => void;
}

const App: React.FC<AppProps> = ({ onBackToSelection }) => {
  ...
  const [vocabSort, setVocabSort] = useState<'pinyin' | 'hanzi' | 'english'>('pinyin');

  // Load saved words on mount
  useEffect(() => {
    const saved = localStorage.getItem('mandarin_echo_vocab');
    if (saved) {
      try {
  ...
  const saveWord = (word: Word) => {
    ...
    // Check if word already exists to avoid duplicates
    const exists = savedWords.some(w => w.pinyin === word.pinyin && w.hanzi === word.hanzi);
    if (!exists) {
      const newWords = [...savedWords, word];
      setSavedWords(newWords);
      localStorage.setItem('mandarin_echo_vocab', JSON.stringify(newWords));
    }
  ...
  const updateProgress = useCallback(() => {
    if (audioControllerRef.current && !isDragging) {
      ...
      if (status === 'playing' || status === 'paused') {
        ...
        if (appState === AppState.FULL_LISTEN) setIsFullStoryDone(true);
      }
    }
  ...
  const triggerListeningSequence = useCallback(async (isFull: boolean = false, index?: number, storyOverride?: Story, isQuestion: boolean = false) => {
    stopCurrentAudio();
    ...
    try {
      ...
      if (isFull) {
        ...
        // true = slow
        const result = await prepareConversationTTS(segments, true, activeStory.id);
        ...
        // Single Sentence / Question Logic
        let textToPlay = "";
        let gender: 'male' | 'female' = 'female';

        if (isQuestion) {
          const qIdx = index !== undefined ? index : currentQuestionIndex;
          textToPlay = activeStory.questions[qIdx].questionMandarin;
          // Alternate question voice for variety too
          gender = qIdx % 2 === 0 ? 'female' : 'male';
        } else {
          const sIdx = index !== undefined ? index : currentSentenceIndex;
          textToPlay = activeStory.sentences[sIdx].mandarin;
          gender = sIdx % 2 === 0 ? 'female' : 'male';
        }

        controller = await prepareMandarinTTS(textToPlay, true, gender, activeStory.id, !isQuestion);
      }
  ...
  const startPractice = (story: Story) => {
    setAppState(AppState.FULL_LISTEN);
    setSelectedStory(story);
    triggerListeningSequence(true, 0, story);
  };

  const handleContinueToSentences = () => {
    setAppState(AppState.SENTENCE_LISTEN);
  ...
  const handleNextSentence = () => {
    ...
    if (currentSentenceIndex < selectedStory.sentences.length - 1) {
      ...
      setAppState(AppState.COMPREHENSION);
      ...
      <div className="w-full max-w-md mt-6 px-4 group flex flex-col items-center">
        ...
        <div className="relative w-full h-6 flex items-center cursor-pointer">
          ...
          <div className="absolute inset-x-0 h-full pointer-events-none">
            <div
              className="absolute h-[3px] bg-violet-400 top-1/2 -translate-y-1/2 rounded-full left-0 transition-all duration-75 ease-out"
              style={{ width: `${progressPercent}%` }}
            />
            <div
              className={`absolute h-4 w-4 bg-violet-400 rounded-full border-2 border-violet-700 shadow-[0_0_15px_rgba(139,92,246,0.5)] top-1/2 -translate-y-1/2 transition-all duration-75 ease-out ${isDragging ? 'scale-125' : 'scale-100'}`}
    ...
    <div className="p-8 space-y-8 animate-in fade-in duration-300">
      <div className="flex items-center justify-between border-b border-slate-800/50 pb-6">
        <div className="flex items-center gap-4">
          {onBackToSelection && (
            <button
              ...
              className="p-3 rounded-2xl bg-slate-900 border border-slate-800 text-slate-500 hover:text-white hover:border-violet-500/50 transition-all active:scale-95 group"
        ...
        <div className="flex gap-2">
          <button onClick={() => setAppState(AppState.VOCABULARY)} className="bg-slate-800/50 hover:bg-violet-500/20 text-slate-400 hover:text-violet-300 transition-colors p-2 rounded-xl border border-slate-700 hover:border-violet-500/30">
      ...
      <div className="grid gap-6">
        {STORIES.map((story) => (
          <button key={story.id} onClick={() => startPractice(story)} className="group flex items-center justify-between p-6 rounded-3xl border border-slate-800 bg-slate-800/20 hover:border-violet-400/30 hover:bg-slate-800/40 transition-all text-left shadow-lg">
            <div>
              <h3 className="text-xl font-bold text-slate-300 group-hover:text-violet-300 transition-colors">{story.title}</h3>
              <p className="text-slate-400 text-sm mt-1 leading-relaxed">{story.description}</p>
            </div>
            <div className="bg-violet-400/20 p-3 rounded-full text-violet-200 opacity-30 group-hover:opacity-100 transition-all transform group-hover:translate-x-1 border border-violet-400/20">
  ...
  const renderFullListen = () => (
    <div className="flex-1 flex flex-col p-8 animate-in fade-in duration-300 relative">
      ...
      <div className="flex justify-between items-center mb-8">
        <button onClick={() => { setAppState(AppState.SELECTING); stopCurrentAudio(); }} className="text-slate-500 hover:text-violet-300 p-2 -ml-2 transition-colors flex items-center gap-2 group">
      ...
      <div className="flex-1 flex flex-col items-center justify-center">
        <div className={`w-48 h-48 rounded-full border-[6px] flex items-center justify-center text-slate-400 text-3xl font-black shadow-2xl transition-all duration-300 select-none
          ${isPulsating ? 'bg-violet-400/30 border-violet-400/40 animate-pulsate' : (isAudioPlaying && !isPaused) ? 'bg-violet-400/20 border-violet-400/20 text-violet-200 shadow-[0_0_80px_rgba(139,92,246,0.15)]' : 'bg-slate-800/40 border-slate-800/60'}`}>
        ...
        {isFullStoryDone && !isAudioPlaying && !isPulsating && (
          <>
            ...
            <button onClick={handleContinueToSentences} className="flex-[2] relative overflow-hidden group rounded-3xl p-[2px] active:scale-95 h-[72px]">
              <div className="absolute inset-0 bg-gradient-to-r from-violet-600 to-indigo-600 opacity-60 group-hover:opacity-100 transition-opacity blur-md" />
              <div className="relative h-full w-full bg-[#1e1e32] rounded-[22px] flex items-center justify-center border border-violet-400/20 overflow-hidden">
    ...
    return (
      <div className="flex-1 flex flex-col p-8 animate-in fade-in duration-300 relative">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center gap-4">
            <div className="text-[10px] font-black uppercase tracking-[0.4em] text-violet-400/60 hidden sm:block">Challenge {currentQuestionIndex + 1} / 4</div>
            <div className="flex gap-1">
              {[0, 1, 2, 3].map(idx => (
                <button
                  key={idx}
                  onClick={() => handleJumpToQuestion(idx)}
                  className={`h-1.5 rounded-full transition-all outline-none ${idx === currentQuestionIndex ? 'bg-violet-400 w-10' : idx < currentQuestionIndex ? 'bg-violet-900 w-6 hover:bg-violet-800' : 'bg-slate-800 w-6 hover:bg-slate-700'}`}
                />
              ))}
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center">
          <div className={`w-32 h-32 rounded-full border-4 flex items-center justify-center text-slate-400 text-xl font-black transition-all duration-300 mb-8
            ${isPulsating ? 'bg-violet-400/30 border-violet-400/40 animate-pulsate' : isAudioPlaying ? 'bg-violet-400/10 border-violet-400/20 text-violet-200' : 'bg-slate-800/20 border-slate-800/40'}`}>
          ...
          {hasAnswered && (
            <div className="mt-8 w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
              <p className="text-slate-400 text-xs text-center mb-6 leading-relaxed bg-violet-400/5 p-4 rounded-xl border border-violet-400/10">
                <span className="text-violet-300 font-black uppercase tracking-widest text-[10px] block mb-1">Key Takeaway</span>
                {question.answerExplanation}
              </p>
              <button
                onClick={handleNextQuestion}
                className="w-full bg-violet-500 text-white font-black py-4 rounded-2xl transition-all shadow-lg active:scale-95 uppercase tracking-widest text-xs"
              >
  ...
  const renderSentenceListen = () => (
    <div className="flex-1 flex flex-col p-8 animate-in fade-in duration-300 relative">
        ...
        <div className="flex gap-2">
          {selectedStory?.sentences.map((_, idx) => (
            <button
              ...
              className={`h-2 rounded-full transition-all duration-500 outline-none ${idx === currentSentenceIndex ? 'bg-violet-400/60 w-16 shadow-[0_0_15px_rgba(139,92,246,0.3)]' : 'bg-slate-800 w-10 hover:bg-slate-700'}`}
            />
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center">
        ...
        <span className="mt-4 text-violet-400/60 font-black uppercase text-[10px] tracking-[0.5em]">Sentence {currentSentenceIndex + 1} / 4</span>
        {renderPlaybackControls()}
          ...
          {showTranslation && selectedStory && (
            <div className="w-full bg-[#0d1221] p-10 rounded-[48px] border border-slate-800/30 animate-in fade-in slide-in-from-bottom-6 duration-700 text-center shadow-2xl relative overflow-hidden">
              <div className="relative z-10 flex flex-col items-center gap-6">
                <p className="text-3xl font-light text-slate-200 tracking-wider leading-relaxed">{selectedStory.sentences[currentSentenceIndex].mandarin}</p>
                <div className="flex flex-wrap justify-center gap-y-2 border-y border-slate-800/20 py-3 px-4 items-center">
                  {selectedStory.sentences[currentSentenceIndex]?.words && selectedStory.sentences[currentSentenceIndex].words.length > 0 ? (
                    (() => {
                      const sentence = selectedStory.sentences[currentSentenceIndex];
                      let remainingPinyin = sentence.pinyin;
                      const elements: React.ReactNode[] = [];

                      sentence.words.forEach((word, index) => {
                        const searchWord = word.pinyin.replace(/\s+/g, '').toLowerCase();
                        // Find the word by ignoring spaces in the remaining pinyin
                        let matchIndex = -1;
                        let matchLength = 0;

                        // We need a fuzzy match because remainingPinyin might have spaces that word.pinyin doesn't,
                        // e.g. "jiā yìdiǎndiǎn" might be in words, but "jiā yì diǎn diǎn" in pinyin.
                        // Actually, standard indexOf is safer, but we should strip punctuation for search maybe?
                        // Let's do a simple indexOf first.
                        matchIndex = remainingPinyin.toLowerCase().indexOf(word.pinyin.toLowerCase());

                        if (matchIndex !== -1) {
                          matchLength = word.pinyin.length;
                        } else {
                          // Try removing spaces
                          const compactRemaining = remainingPinyin.replace(/\s+/g, '').toLowerCase();
                          const compactMatchIndex = compactRemaining.indexOf(searchWord);
                          if (compactMatchIndex !== -1) {
                            // Map back to original string index
                            let compactCounter = 0;
                            for (let i = 0; i < remainingPinyin.length; i++) {
                              if (remainingPinyin[i] !== ' ') {
                                if (compactCounter === compactMatchIndex) {
                                  matchIndex = i;
                                }
                                if (compactCounter === compactMatchIndex + searchWord.length - 1) {
                        ...
                        if (matchIndex !== -1) {
                          ...
                          const exactPinyinStr = remainingPinyin.substring(matchIndex, matchIndex + matchLength);
                          elements.push(
                            <WordSpan
                              key={`word-${index}`}
                              word={{ ...word, pinyin: exactPinyinStr }}
                              onSave={saveWord}
                              isSaved={savedWords.some(w => w.pinyin === word.pinyin && w.hanzi === word.hanzi)}
                            />
                          );

                          remainingPinyin = remainingPinyin.substring(matchIndex + matchLength);
                        } else {
                          elements.push(
                            <WordSpan
                              key={`word-fallback-${index}`}
                              word={word}
                              onSave={saveWord}
                              isSaved={savedWords.some(w => w.pinyin === word.pinyin && w.hanzi === word.hanzi)}
                            />
      ...
      <div className="mt-8 w-full flex flex-col items-center gap-6">
        {!isPulsating && !isAudioPlaying && (
          <button onClick={() => setShowTranslation(!showTranslation)} className={`px-8 py-3 rounded-full font-black text-[10px] uppercase tracking-widest transition-all border active:scale-95 flex items-center gap-2 ${showTranslation ? 'bg-violet-500/10 border-violet-500/40 text-violet-300' : 'bg-slate-800/40 border-slate-800 text-slate-500'}`}>
          ...
          {!isPulsating && !isAudioPlaying ? (
            <>
              ...
              <button onClick={() => triggerListeningSequence(false)} className="group flex items-center gap-3 bg-slate-800/20 hover:bg-slate-800/40 text-slate-300 border border-slate-700/50 px-8 py-4 rounded-3xl transition-all active:scale-95 shadow-xl">
                <svg className="h-5 w-5 text-violet-400 group-hover:rotate-180 transition-transform duration-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
                <span className="font-black uppercase text-xs tracking-widest">Repeat</span>
              </button>
              <button onClick={handleNextSentence} className="w-14 h-14 rounded-2xl flex items-center justify-center text-slate-300 bg-violet-500/10 hover:bg-violet-500/20 active:scale-90 border border-violet-500/20"><svg className="h-8 w-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M9 5l7 7-7 7" /></svg></button>
            </>
          ) : (
  ...
  return (
    <Layout>
      {appState === AppState.SELECTING && renderSelection()}
      {appState === AppState.FULL_LISTEN && renderFullListen()}
      {appState === AppState.SENTENCE_LISTEN && renderSentenceListen()}
      {appState === AppState.COMPREHENSION && renderComprehension()}
      {appState === AppState.VOCABULARY && (
        <div className="flex-1 flex flex-col p-8 animate-in fade-in duration-300 w-full">
          <div className="flex justify-between items-center mb-8 border-b border-slate-800/50 pb-6">
            <button onClick={() => setAppState(AppState.SELECTING)} className="text-slate-500 hover:text-violet-300 -ml-2 p-2 flex items-center gap-2 group">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M15 19l-7-7 7-7" /></svg>
          ...
          <div className="flex justify-end mb-6 gap-2">
            <button onClick={() => setVocabSort('pinyin')} className={`text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest border transition-all ${vocabSort === 'pinyin' ? 'bg-violet-500/20 border-violet-500/50 text-violet-300' : 'border-slate-800 text-slate-500'}`}>Pinyin</button>
            <button onClick={() => setVocabSort('hanzi')} className={`text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest border transition-all ${vocabSort === 'hanzi' ? 'bg-violet-500/20 border-violet-500/50 text-violet-300' : 'border-slate-800 text-slate-500'}`}>Hanzi</button>
            <button onClick={() => setVocabSort('meaning')} className={`text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest border transition-all ${vocabSort === 'meaning' ? 'bg-violet-500/20 border-violet-500/50 text-violet-300' : 'border-slate-800 text-slate-500'}`}>English</button>
          </div>
          ...
          <div className="grid gap-3 overflow-y-auto max-h-[60vh] pr-2 custom-scrollbar">
            {savedWords.length === 0 ? (
              ...
              savedWords.slice().sort((a, b) => a[vocabSort].localeCompare(b[vocabSort])).map((word, idx) => (
                <div key={idx} className="bg-slate-800/20 border border-slate-800 rounded-2xl p-4 flex items-center justify-between group hover:bg-slate-800/40 hover:border-violet-500/20 transition-all">
                  <div className="flex items-center gap-4">
                    {vocabSort === 'pinyin' && (
                      <>
                        <div className="text-violet-400 font-bold font-mono min-w-[80px]">{word.pinyin}</div>
                        <div className="text-slate-200 text-xl font-light min-w-[60px] border-l border-slate-700 pl-4">{word.hanzi}</div>
